...
if (!function_exists('getBaseProductName')) {
    /**
     * Extract base product name from full product name
     *
     * Removes package type suffixes like "Premium", "Enterprise", "Pro", etc.
     * Used to ensure consistent product name usage across API calls.
     *
     * @param string $fullProductName Full product name (e.g., "Sessner Premium")
     * @return string Base product name (e.g., "Sessner")
     *
     * @example
     * getBaseProductName('Sessner Premium')           => 'Sessner'
     * getBaseProductName('Sessner Enterprise')        => 'Sessner'
     * getBaseProductName('Sessner - Premium - Monthly') => 'Sessner'
     * getBaseProductName('MyApp Pro')                 => 'MyApp'
     */
    function getBaseProductName(string $fullProductName): string
    {
        $fullProductName = trim($fullProductName);

        // List of common package type suffixes to remove
        $packageTypes = [
            'Premium',
            'Enterprise',
            'Professional',
        ...
        // Remove common separators and package types
        $baseName = $fullProductName;

        // First, split by common separators (-, |, –, etc.)
        $parts = preg_split('/[\s\-–|]+/', $baseName);

        if (count($parts) > 1) {
            // Check if any part matches a known package type
            $cleanedParts = [];
            foreach ($parts as $part) {
                $part = trim($part);
                if (!in_array($part, $packageTypes, true) &&
                    !in_array(ucfirst(strtolower($part)), $packageTypes, true)) {
                    // Not a package type, keep it
                    if (!empty($part) && !preg_match('/^(monthly|yearly|annual|weekly|daily|quarterly)$/i', $part)) {
                        $cleanedParts[] = $part;
                    }
                }
            }
