        <?php if(!isset($myConfig)) {$myConfig = getMyConfig('', $userData->id); } ?>
        <meta charset="utf-8" />        
        ...
        if($myConfig['defaultTheme'] === 'system') {
            // For system theme, we'll get the actual scheme from JavaScript ?>
                ...
                function update_color_scheme() {
                    ...
                    let cookieValue = "color_scheme=" + encodeURIComponent(scheme) + "; " + 
                                        expires + "; " + 
                                        "path=/; " + 
                                        "secure; " + 
                                        "samesite=Strict";
        ...
        <?php }
                ...
                <script type="text/javascript">
               // Function to update the color_scheme cookie with user's preference
                function update_color_scheme() {
                    // Set expiration date to 1 year from now
                    const expiryDate = new Date();
                	expiryDate.setFullYear(expiryDate.getFullYear() + 1);
                    ...
                    // Build cookie string with all parameters
                    let cookieValue = "color_scheme=" + encodeURIComponent('<?= $myConfig['defaultTheme'] ?>') + "; " + 
                                        expires + "; " + 
                                        "path=/; " + 
                                        "secure; " + 
                                        "samesite=Strict";
        ...
        <!-- favicon -->
        <link rel="shortcut icon" href="<?= $myConfig['appIcon'] ?>" />
        <!-- Css -->
        <link href="<?= base_url('assets/libs/simplebar/simplebar.min.css') ?>" rel="stylesheet">
        <!-- Bootstrap Css -->
        <link href="<?= $bootstrapCSS ?>" class="theme-opt" rel="stylesheet" type="text/css" />
        <!-- Icons Css -->
        <link href="<?= base_url('assets/libs/@mdi/font/css/materialdesignicons.min.css') ?>" rel="stylesheet" type="text/css">
        <link href="<?= base_url('assets/css/icons' . $direction . '.min.css') ?>" rel="stylesheet" type="text/css" />
        <link href="<?= base_url('assets/libs/@iconscout/unicons/css/line.css') ?>" type="text/css" rel="stylesheet" />
        <!-- Style Css-->
        <link href="<?= $styleCSS ?>" class="theme-opt" rel="stylesheet" type="text/css" />

        <style>
        /* Add this style to hide the vertical scrollbar */
        .container-profilepic {
                width: 80px;
                height: 80px;
        }
        .photo-preview {
                background-image: url( <?= base_url('writable/uploads/user-avatar/' . $avatar) ?> );
