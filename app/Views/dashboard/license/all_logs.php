        ...
        <div class="row">
            <div class="col-12 mt-4">
                ...
                <div class="border-0 mb-3">
                    ...
                    <button class="mx-auto btn btn-soft-danger" id="delete-all-logs"><i class="uil uil-times"></i> <?= lang('Pages.Clear_All_Logs') ?></button>
                </div>
...
<?= $this->section('scripts') ?>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {            
            const searchParam = getUrlParameter('s');
            
            /*******************
             * Handle dataTables
             ******************/   
            const table = $('#license-list-table').DataTable({
                "ajax": {
                    "url": "<?= base_url('api/license/logs/all/' . $myConfig['General_Info_SecretKey']) ?>",
                    "type": "GET",
                    "dataSrc": "",
                ...
                "columns": [
                    ...
                    { "data": "action", "title": '<?= lang('Pages.Action') ?>' },
                    { "data": "time", "title": '<?= lang('Pages.Date_and_Time') ?>', "className": "text-muted", },
                    { "data": "source", "title": '<?= lang('Pages.Source') ?>', "className": "text-muted", },
                ],
                "columnDefs": [
                    {
                        "targets": 0,
                    ...
                    {
                        "targets": 2, // Targets the license_key column
                    ...
                    {
                        "targets": 3, // Targets the license_key column
                        "render": function(data, type, row) {
                            return `<span class="text-${row.is_valid === 'yes' ? 'success' : 'danger'}">${data}</span>`;
                        }
                    },
                    {
                        "targets": "_all",
                            ...
                            if (type === 'display' && data) {
                                if (data === '0000-00-00 00:00:00' || data === '') {
                                    ...
                                    if (!isNaN(date.getTime())) {
                                        // Format the date to display only as "Y-m-d"
                                        return formatDateTime(data);
                                    } else {
                                        // If the data is not a valid date string, return it as is
                                        return data;
                                    }
                                }
                            } else {
            ...
            // Apply search if 's' parameter exists
            if (searchParam) {
                table.search(searchParam).draw();

                // Optional: Also set the input box manually in case DataTables doesn't update it
            ...
            ***************************/
            $('#delete-license-submit').on('click', function (e) {
                ...
                if (confirmDelete) {
                    ...
                    $.ajax({
                        url: '<?= base_url('api/license/delete/logs/' . $myConfig['Manage_License_SecretKey']) ?>',
                        method: 'POST',
                        data: data,
                        processData: false,
                        contentType: false,
