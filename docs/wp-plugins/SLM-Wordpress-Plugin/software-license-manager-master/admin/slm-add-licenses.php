...
function slm_add_licenses_menu()
{
    ...
    if (isset($_POST['save_record'])) {
        ...
        if ( isset($_POST['item_reference']) && ("" == trim($_POST['item_reference'])) ) {            
            $lic_item_ref   = trim($_POST['item_reference']);
        } else {
            $lic_item_ref   = $product_ref;
        }
        ...
        if ($_POST['lic_type'] == 'lifetime') {
            ...
            $expiry_date    = isset($_POST['date_expiry']) ? $_POST['date_expiry'] : date('dd/mm/yyyy', strtotime('-1 day'));
        }
        ...
        if (empty($id)) {
            ...
            // MERAF Prod Panel: Start sending email templates
            else {
                $merafProdPanelIntegration = SLM_MERAF_ProdPanel::merafProdPanelIntegration();
                if ($merafProdPanelIntegration) {
                    // Prepare the data to be sent
                    $licenseKey = $license_key;
                    $licenseType = $lic_type;
                    $clientFullName = $first_name . ' ' . $last_name;
                    $clientEmail = $email;
                    $whatProduct = $product_ref;
                    
                    $response = SLM_MERAF_ProdPanel::sendEmailtoClient_new_license($licenseKey, $licenseType, $clientFullName, $clientEmail, $whatProduct);
                    ...
                    if (json_decode($response) === null && json_last_error() !== JSON_ERROR_NONE) {
                        ...
                        $response = [
                            'success' => false,
                            'status'  => 0,
                            'msg'     => 'Encountered a problem upon sending email to the client!',
                        ];
                    } else {
                    ...
                    if($response['success'] === true) {
                        $emailNotification = '<div class="alert alert-dismissible alert-primary" role="alert"> '.$response['msg'].'</div>';
                    }
                    else {
                        $emailNotification = '<div class="alert alert-dismissible alert-danger" role="alert">'.$response['msg'].'</div>';
                    }
                }
            }
            // MERAF Prod Panel: End sending email templates      
        } else {
 ...
 <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.rtl.min.css" integrity="sha384-gXt9imSW0VcJVHezoNQsP+TNrjYXoGcrqBZJpry9zJt8PCQjobwmhMGaDHTASo9N" crossorigin="anonymous">
    ...
    <div id="container-2" class="container slm-container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            echo '<div class="alert alert-dismissable alert-primary" role="alert"> <strong>'.__('Done! ', 'softwarelicensemanager').'</strong>' .__('License was updated successfully.', 'softwarelicensemanager'). '</div>';
                                        ...
                                        <div class="col-9 sml-col-left">
                                            <form method="post" class="slm_license_form row" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
                                                ...
                                                if ($id != '') {
                                                    ...
                                                    if($merafProdPanelIntegration) { 
                                                        $license_key      = SLM_MERAF_ProdPanel::meraf_prodpanel_generateLicense($txn_id, $product_ref);
                                                    }
                                                    else {
                                                ...
                                                /***************************************
                                                 * Integration of MERAF Production Panel
                                                 **************************************/
                                                ...
                                                <div class="tab-content col-md-12" id="slm_manage_licenseContent">
                                                        ...
                                                        <div class="license col-full">
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="form-group col-md-6">
                                                                    ...
                                                                    <select name="lic_type" class="form-control required" required>
                                                                        <option value="" > <?php _e('- SELECT -','softwarelicensemanager'); ?> </option>
                                                                        <?php if($merafProdPanelIntegration) { ?>
                                                                            <option value="subscription" <?php if ($lic_type == 'subscription') {
                                                                                                                echo 'selected="selected"';
                                                                                                            } ?>> <?php _e('Trial/Subscription','softwarelicensemanager'); ?> </option>
                                                                        <?php } else { ?>
                                                                            <option value="subscription" <?php if ($lic_type == 'subscription') {
                                                                                                            echo 'selected="selected"';
                                                                        ...
                                                                        <option value="lifetime" <?php if ($lic_type == 'lifetime') {
                                                                                                        echo 'selected="selected"';
                                                                                                    } ?>> <?php _e('Lifetime','softwarelicensemanager'); ?></option>
                                                                    </select>
                                                    ...
                                                    <div class="tab-pane fade show " id="productinfo" role="tabpanel" aria-labelledby="productinfo-tab">
                                                        ...
                                                        <div class="col-full">
                                                            ...
                                                            <div class="form-group">
                                                                <?php 
                                                                if(SLM_MERAF_ProdPanel::merafProdPanelIntegration()) { 
                                                                    $productNames = SLM_MERAF_ProdPanel::meraf_prodpanel_products();
                                                                    $productVariations = SLM_MERAF_ProdPanel::meraf_prodpanel_variations();
                                                                    $productNameWithVariations = SLM_MERAF_ProdPanel::meraf_prodpanel_products_with_variations();
                                                                ?>
                                                                    <label for="product_ref">Type</label>
                                                                    <select name="product_ref" class="form-control required" required>
                                                                        <option value="" >- Select -</option>

                                                                        <?php
                                                                        
                                                                        foreach($productNameWithVariations as $productNameWithVariation) {
                                                                            $selectedOption = '';
                                                                            if($productNameWithVariation == $product_ref) {
                                                                                $selectedOption = 'selected="selected"';
                                                                            }
                                                                            echo '<option value="' . $productNameWithVariation . '" '.$selectedOption.'>' . $productNameWithVariation . '</option>';
                                                                        }                                                                       
                                                                        ?>                                                                      
                                                                    </select>
                                                                <?php } else { ?>
                                                            ...
                                                            <div class="row">
                                                                <div class="form-group col-md-6">
                                                                    <?php if($merafProdPanelIntegration) { ?>
                                                                        <label for="txn_id"><?php _e('Reference / Transaction ID','softwarelicensemanager'); ?></label>
                                                                        <input name="txn_id" type="text" class="form-control" id="txn_id required" required value="<?= $txn_id ?>" />
                                                                    <?php } else { ?>
                                                                        <label for="txn_id"><?php _e('Unique Transaction ID','softwarelicensemanager'); ?></label>
                                                                        <input name="txn_id" type="text" class="form-control" id="txn_id required" required value="<?php echo $txn_id; ?>" />
                                                                    <?php } ?>                                                                    
                                                                ...
                                                                <div class="form-group col-md-6">
                                                                    ...
                                                                    <?php if($merafProdPanelIntegration) { ?>
                                                                        <input name="purchase_id_" class="form-control required" required type="text" id="purchase_id_" value="<?= $purchase_id_ ?>" size="8" />
                                                                    <?php } else { ?>                                                                        
        ...
        jQuery(document).ready(function() {
            ...
            jQuery(".save_lic").click(function(event) {
                ...
                if (form[0].checkValidity() === false) {
                    <?php if($merafProdPanelIntegration) { ?>
                        jQuery('#license-tab').css("color", "red");
                        jQuery('#productinfo-tab').css("color", "red");
                    <?php } ?>
                ...
                jQuery('.deactivate_lic_key').click(function(event) {
                    ...
                    jQuery.get('<?php echo esc_url(home_url('/')); ?>' + 'wp-admin/admin-ajax.php?action=del_activation&id=' + id + '&activation_type=' + activation_type, function(data) {
                        if (data == 'success') {
                            ...
                            jQuery('.slm_ajax_msg').html('<div class="alert alert-dismissible alert-primary" role="alert"> License key was deactivated! </div>');
                        } else {
                            jQuery('.slm_ajax_msg').html('<div class="alert alert-dismissible alert-danger" role="alert"> License key was not deactivated! </div>');
                        }
                    });
                });
            });
