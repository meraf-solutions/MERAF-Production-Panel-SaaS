 ...
 **************************************/
class SLM_MERAF_ProdPanel
{

    static function merafProdPanelIntegration()
    {
        $merafProdPanelIntegration = SLM_Helper_Class::slm_get_option('slm_meraf_prod_panel') ? true : false;
        return $merafProdPanelIntegration;
    }

    static function meraf_prodpanel_products()
    {
        $merafProdPanelURL = SLM_Helper_Class::slm_get_option('slm_meraf_prod_panel');

        $curl = curl_init($merafProdPanelURL.'api/product/all');
    ...
    static function meraf_prodpanel_products_with_variations()
    {
        $merafProdPanelURL = SLM_Helper_Class::slm_get_option('slm_meraf_prod_panel');

        $curl = curl_init($merafProdPanelURL.'api/product/with-variations');
        curl_setopt_array($curl, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_HTTPAUTH => CURLAUTH_BASIC,
        ...
        if ($error) {
            // Log cURL error
            error_log('cURL Error: '.$error);
            return 'Unable to retrieve the product list with variations!';
        }        

        return json_decode($result, true);
    }

    static function meraf_prodpanel_variations()
    {
        $merafProdPanelURL = SLM_Helper_Class::slm_get_option('slm_meraf_prod_panel');

        $curl = curl_init($merafProdPanelURL.'api/variation/all');
        curl_setopt_array($curl, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_HTTPAUTH => CURLAUTH_BASIC,
        ...
        if ($error) {
            // Log cURL error
            error_log('cURL Error: '.$error);
            return 'Unable to retrieve the product variations!';
        }        

        return json_decode($result, true);
    } 
    
    static function meraf_prodpanel_generateLicense($domain, $key)
    {

        $merafProdPanelURL = SLM_Helper_Class::slm_get_option('slm_meraf_prod_panel');

        $curl = curl_init($merafProdPanelURL.'api/license/generate');
        curl_setopt_array($curl, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_SSL_VERIFYPEER => false,
            CURLOPT_HTTPAUTH => CURLAUTH_BASIC,
        ...
        if ($error) {
            // Log cURL error
            error_log('cURL Error: '.$error);
            return 'Unable to generate license key from the production panel!';
        }        

        return json_decode($result, true); 
    }

    static function sendEmailtoClient_new_license($licenseKey, $licenseType, $clientFullName, $clientEmail, $whatProduct)
    {
        // Prepare the data before API call
        $clientFullName = urlencode($clientFullName);
        $clientEmail = urlencode($clientEmail);
        $whatProduct = urlencode($whatProduct);
        
        $merafProdPanelURL = SLM_Helper_Class::slm_get_option('slm_meraf_prod_panel');
        $apiURL = $merafProdPanelURL . 'send-email-new-license/'. $licenseKey . '/' . $licenseType . '/' . $clientFullName . '/' . $clientEmail . '/' . $whatProduct;
        ...
        if ($error) {
            // Log cURL error
            error_log('cURL Error: '.$error);
            return 'Failed to send email: cURL Error';
        }        
...
// $meraf_helper = new SLM_MERAF_ProdPanel();
